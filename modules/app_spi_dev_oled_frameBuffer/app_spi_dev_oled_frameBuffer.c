...
static int dc_pin_num; // DC 引脚号
 ...
 unsigned char font8x8_basic_dev_frameBuffer[][8] = {
...
void OLED_DIsp_Clear_dev_frameBuffer(void)  
{
    ...
    for ( y = 0; y < 8; y++)
    {
        OLED_DIsp_Set_Pos_dev_frameBuffer(0, y); // 设置光标位置; 获取 每一行

        oled_write_datas_dev_frameBuffer(buf, 128); // 写入数据
    }
}
// 设置光标位置
void OLED_DIsp_Set_Pos_dev_frameBuffer(int x, int y){
...
void OLED_DIsp_Char_dev_frameBuffer(int x, int y, unsigned char c){
    int i = 0;
    unsigned char *p = font8x8_basic_dev_frameBuffer[c - 32]; // 
    
    // const unsigned char *p = oled_asc2_8x16[c - 32]; // 获取字符对应的字模
    // 设置光标位置
    OLED_DIsp_Set_Pos_dev_frameBuffer(x, y); // 设置光标位置
    // 设置写入数据
    // oled_write_datas_dev_frameBuffer(&p[0], 8); // 写入字符的字模数据

    oled_write_datas_dev_frameBuffer(p, 8); // 写入 8 字节字体数据
    // 设置光标位置
...
void OLED_DIsp_String_dev_frameBuffer(int x, int y, char *str){
    ...
    {
        // 获取字符对应的字模
        OLED_DIsp_Char_dev_frameBuffer(x, y, str[j]);// x y 是坐标，str[j] 是字符
        // 更新 x 和 y 坐标
...
void OLED_DIsp_Test_dev_frameBuffer(void){
    int i;
    OLED_DIsp_String_dev_frameBuffer(0, 0, "kingnan");
	OLED_DIsp_String_dev_frameBuffer(0, 1, "Guo");
	OLED_DIsp_String_dev_frameBuffer(0, 2, "heart.com");
	OLED_DIsp_String_dev_frameBuffer(0, 3, "1992-11-4");
    OLED_DIsp_String_dev_frameBuffer(0, 4, "a1b2c3d4");
}



 /** main /dev/spidevB.D <DC_Pin_number> */
int app_spi_dev_oled_frameBuffer_init(int argc, char *argv[]){
    ...
    OLED_DIsp_Clear_dev_frameBuffer();
    // 显示测试内容
    OLED_DIsp_Test_dev_frameBuffer();
    // SPI 初始化代码可以在这里添加
    close(fd);
    return 0;
}
