...
static int virtual_gpio_probe(struct platform_device *pdev)
{
    ...
    /** 提供 新的  request  函数  */
    global_virtual_gpio->request = gpiochip_generic_request;// gpiochip_generic_request 是 request 用来申请 gpio 的
    ...
    if (ret) {
        dev_err(&pdev->dev, "Failed to read ngpios property: %d\n", ret);
        return ret;
    }
    global_virtual_gpio->ngpio = val; // gpio 的数量，从设备树里获得

    /** 3 注册  gpio_chip */
    ret = devm_gpiochip_add_data(&pdev->dev, global_virtual_gpio, NULL);
    if (ret) {
