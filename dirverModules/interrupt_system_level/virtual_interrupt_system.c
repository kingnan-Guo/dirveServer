...
static void virtual_interrupt_irq_mask(struct irq_data *data)
{
    ...
    irq_chip_mask_parent(data); // 屏蔽 父节点 中断 
}

static void virtual_interrupt_irq_unmask(struct irq_data *data)
{
    printk("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
    irq_chip_unmask_parent(data); // 解除 父节点 中断 屏蔽
    // 这里可以添加一些其他的操作，比如设置寄存器等
    // 解除屏蔽后，父节点的中断控制器会开始处理这个中断
}
 ...
 * 
 */
static int virtual_interrupt_domain_translate(struct irq_domain *d, struct irq_fwspec *fwspec, unsigned long *hwirq, unsigned int *type)
{
    printk("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
...
static int virtual_interrupt_probe(struct platform_device *pdev)
{
    ...
    // 从设备树 中 获取 static u32 upper_hwirq_base;
    if (of_property_read_u32(np, "upper_hwirq_base", &upper_hwirq_base)) {
        printk(KERN_ERR "Failed to read upper_hwirq_base from device tree\n");
        return -EINVAL;
    }
    ...
    if (!parent) {
        printk(KERN_ERR "Failed to find parent interrupt node\n");
        return -ENODEV;
    }

    parent_domain = irq_find_host(parent); // 查找父域， 也就是查找设备树的节点对应的 irq_domain
    if (!parent_domain) {
        printk(KERN_ERR "Failed to find parent IRQ domain\n");
        return -ENODEV;
    }
    ...
    if (!virtual_interrupt_domain) {
        printk(KERN_ERR "Failed to add IRQ domain\n");
        return -ENOMEM;
    }
...
static int virtual_interrupt_remove(struct platform_device *pdev)
{
    printk("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
    if (virtual_interrupt_domain) {
        irq_domain_remove(virtual_interrupt_domain);
        virtual_interrupt_domain = NULL;
    }
    return 0;
}
