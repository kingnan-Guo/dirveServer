...
static int __init my_test_device_init(void) {
   ...
   */
    ...
    snprintf(device_name_buf, sizeof(device_name_buf), "%s_%s", DEVICE_NAME, "_class");
    my_test_device_class = class_create(device_name_buf);
    ...
    device_create(my_test_device_class, NULL, MKDEV(major, minor), NULL, device_name_buf);//创建 文件系统 的设备节点; 应用程序 通过文件系统的设备 节点 访问 硬件  
