...
struct input_device_button_data {
    ...
    unsigned long last_time; /* 每个按键的去抖时间 */
};
...
static int is_buffer_full(struct my_input_device *my_input_dev)
{
    ...
    return (my_input_dev->read_index == next_pos(my_input_dev->write_index));
}
...
static int get_event(struct my_input_device *my_input_dev, struct button_event *event){
    ...
    if(!is_buffer_empty(my_input_dev)){
...
static long my_input_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case IOCTL_GET_EVENT_PATH:
            char event_path[256];
            // if(dev->input_data->input_device->evdev){
            //     snprintf(event_path, sizeof(event_path), "/dev/input/event%d", dev->input_data->input_device->evdev->minor);
            //     if (copy_to_user((char *)arg, event_path, sizeof(event_path))) {
            //         return -EFAULT;
            //     }
            // }
            if (!dev->input_data->input_device->evdev) {
                dev_err(&dev->input_data->input_device->dev, "evdev not ready\n");
                return -ENODEV;
            }
            snprintf(event_path, sizeof(event_path), "/dev/input/event%d", MINOR(dev->input_data->input_device->evdev->dev.devt));
            if (copy_to_user((void __user *)arg, event_path, sizeof(event_path))) {
                dev_err(&dev->input_data->input_device->dev, "Failed to copy event path\n");
                return -EFAULT;
            }
            return 0;
        default:
...
static int input_device_probe(struct platform_device *pdev)
{
    ...
    my_input_dev->read_index = 0;// 初始化读索引
    my_input_dev->write_index = 0;// 初始化写索引
    // 初始化等待队列头
   ...
   // set 1 设置哪一类型事件
    ...
    dev_info(dev, "Registered input device as %s\n", dev_name(&input_device_data->input_device->dev));
...
static int input_device_remove(struct platform_device *pdev)
{
    ...
    input_unregister_device(my_input_dev->input_data->input_device);
